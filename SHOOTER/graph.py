...
def create_grid():
  # creates the 15 x 15 grid that is used within the game
  # used to initialize the graph for the game

  # set of the coordinates of the vertices that are part of the 15 x 15 grid
  vset = set()
  for i in range(15):
    for j in range(15):
      vset.add((i, j))

  # list of all edges in the 15 x 15 grid
  # edges consists of vertices that are adjacent to each other, but are not diagonal 
  # to each other. 
  # The edges create an undirected graph
  elist = []
  for x in range(14):
    for y in range (15):
      elist.append(((x,y), (x+1, y)))
  ...
  for x in range(15):
    for y in range(14):
      elist.append(((x, y),(x , y+1)))  

  for x in range(15):
    for y in reversed(range(1,15)):
      elist.append(((x,y),(x,y-1)))
